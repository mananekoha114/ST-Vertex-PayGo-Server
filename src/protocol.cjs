...
const PLUGIN_VERSION = '0.1.0-dev.0';
...
function validatePreparePayload(body) {
    ...
    if (body.chat_completion_source !== 'vertexai') {
        throw new PluginError(400, 'VERTEX_ONLY', 'Only the SillyTavern Vertex AI source is supported.');
    }
    ...
    const region = requireCanonicalString(body.vertexai_region, 'Vertex AI region', /^(?:global|[a-z0-9][a-z0-9-]*)$/u, 63);
    const authMode = requireCanonicalString(body.vertexai_auth_mode, 'Vertex AI authentication mode', /^(?:express|full)$/u, 16);
    ...
    if ((tier === 'flex' || tier === 'priority') && region !== 'global') {
    ...
    if (body.vertexai_express_project_id !== undefined && body.vertexai_express_project_id !== '') {
