...
function createLoopbackTransport({ ticketStore, upstreamRequest, maxBodyBytes, upstreamTimeoutMs } = {}) {
    const proxyHandler = createProxyHandler({ ticketStore, upstreamRequest, maxBodyBytes, upstreamTimeoutMs });
    let server;
    let baseUrl;
