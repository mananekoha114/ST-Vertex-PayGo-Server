...
SillyTavern 1.18.x 的原生 Vertex AI 请求没有提供 PayGo 服务层级选项。前端扩展负责选择和保存层级，Server Plugin 则在 SillyTavern 服务端完成以下工作：

- 复用当前用户已经保存在 SillyTavern 中的 Vertex AI 认证配置。
- 根据 Standard PayGo-only、Flex 或 Priority 注入对应的 Google 请求头。
- 为单次请求签发短期票据和随机凭据。
- 通过只监听 `127.0.0.1` 的临时 HTTP 端口接收 SillyTavern 服务端转发的请求。
...
- SillyTavern 1.18.x。
- Node.js 20 或更高版本。
- 已配置可用 Vertex AI Express 或完整服务账号认证的 SillyTavern 用户。
- 配套的 `ST-Vertex-PayGo` 前端扩展。

插件启动时会读取 SillyTavern 根目录的 `package.json` 和 `src/endpoints/google.js`。版本不属于 1.18.x，或缺少所需的 Vertex AI 配置接口时，插件会拒绝启动。

## 安装

停止 SillyTavern，将本仓库放入：
...
```text
SillyTavern/public/scripts/extensions/third-party/ST-Vertex-PayGo/
```

本项目没有需要单独安装的运行时 npm 依赖。重新启动 SillyTavern 后，前端扩展应显示 Server Plugin 已就绪。

## 请求流程

1. 前端扩展在生成前向 `/api/plugins/vertex-paygo/prepare` 发送经过 SillyTavern 登录和 CSRF 保护的控制请求。
2. Server Plugin 验证协议、来源、Gemini 模型、区域、认证模式和层级组合。
3. 插件调用 SillyTavern 自身的 Vertex AI 配置接口，取得当前用户对应的 Google 目标地址和认证请求头。
4. 插件生成五分钟有效的一次性票据与随机代理凭据，并返回仅绑定到 `127.0.0.1` 的代理 URL。
5. SillyTavern 服务端把实际生成请求发送到这个本机回送地址，并通过 Bearer 凭据证明它持有该票据。
6. 插件以原子方式验证凭据和完整请求后缀并消费票据，再次验证 Google 目标和请求头，最后将请求转发给 Vertex AI。

浏览器不会直接连接本机回送端口。即使 SillyTavern 部署在远程服务器上，访问 `127.0.0.1` 的仍是 SillyTavern 服务端进程。
...
Standard 且 PayGo-only 关闭时必须使用 SillyTavern 原生 Vertex AI 请求；Server Plugin 会拒绝为这种组合创建代理票据。

## 控制接口

SillyTavern 会把插件路由挂载在 `/api/plugins/vertex-paygo` 下：

- `GET /health`：返回插件版本、协议版本、传输类型和 SillyTavern 兼容信息。
...
这些限制用于缩小 Server Plugin 的权限范围，但不能替代 SillyTavern 本身的账号、网络和文件系统安全配置。
...
- 在 SillyTavern 1.18.x 之外的版本上绕过兼容性检查运行。
...
Flex 本身允许延迟执行。插件不会在 SillyTavern 默认的短超时内提前中止，但上游请求仍会在约 31 分钟后超时。
...
测试覆盖协议和目标校验、请求头策略、SillyTavern 运行时适配、prepare、票据生命周期，以及本机回送代理的成功与失败路径。
