...
2. **鉴权与签发**：插件校验模型、区域与层级后，先按当前用户预留总票据池、单用户未消费票据额度和签发速率，再从宿主获取该用户的 Vertex 认证头。认证失败会释放预留；成功后生成一张**5分钟有效、阅后即焚**的一次性票据（Ticket）和随机 Token，并返回仅限本地监听的代理地址（`127.0.0.1:xxxx`）。
...
- 无效 prepare 和匿名拒绝属于低优先级诊断事件，使用独立的有界预算（默认最多占日志文件的 40%，上限 2 MiB）；该预算耗尽后只丢弃这类噪声，仍保留普通服务端诊断的写入机会。
...
- **用户级隔离**：票据记录所属用户；总池默认最多 256 张、单用户最多 32 张未消费票据，并按用户限制每分钟最多 30 次签发尝试。额度在昂贵的 Google 认证读取之前预留，认证失败会归还占用的票据槽位。
...
显式密钥选择：AI Studio 的 `secret_id` 通过宿主密钥存储按当前用户及 ID 严格读取；找不到时返回错误，不回退默认密钥。Vertex Express/Full 带显式 ID 的请求目前返回 `EXPLICIT_SECRET_UNSUPPORTED`，未指定 ID 时继续使用宿主默认认证。更新配套前端后，预检会发生在最终密钥覆盖完成之后。部署修复后需重启宿主并刷新前端。
